...
def run_scan_task(self, exchange_name, market_type, quote, timeframe, pattern, limit=None):
    ...
    if not pairs:
        self.update_state(state=states.FAILURE, meta={'exc': 'Could not fetch instrument list'})
        raise Ignore()
    ...
    for i, pair in enumerate(pairs_to_scan):
        ...
        if kline_data:
            for pattern_name in patterns_to_check:
                ...
                if is_pattern:
                    found_list.append({
                        "exchange": exchange_adapter.name, "market": market_type, "pair": pair, 
                        "timeframe": timeframe, "status": status, "description": description
                    })
                    break
