import requests
import sys
import time
import numpy as np
from tqdm import tqdm
import argparse

# --- 轉接頭架構 ---
class BaseExchange:
    """交易所轉接頭的基底類別，定義了所有交易所都必須實現的方法。"""
    def __init__(self):
        self.name = "Base Exchange"

    def get_spot_instruments_by_quote(self, quote_currency):
        """獲取指定報價貨幣的現貨交易對列表。"""
        raise NotImplementedError

    def get_kline_data(self, instrument_id, timeframe, limit=100):
        """為指定的交易對獲取 K 線數據。"""
        raise NotImplementedError

class OkxAdapter(BaseExchange):
    """OKX 交易所的轉接頭"""
    def __init__(self):
        super().__init__()
        self.name = "OKX"
        self.instruments_url = "https://www.okx.com/api/v5/public/instruments"
        self.kline_url = "https://www.okx.com/api/v5/market/history-candles"

    def get_spot_instruments_by_quote(self, quote_currency):
        params = {'instType': 'SPOT'}
        try:
            response = requests.get(self.instruments_url, params=params)
            response.raise_for_status()
            data = response.json()
            if data.get('code') == '0':
                instruments_data = data.get('data', [])
                filter_suffix = f'-{quote_currency.upper()}'
                return [item['instId'] for item in instruments_data if item['instId'].endswith(filter_suffix)]
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] OKX API request failed: {e}")
        return None

    def get_kline_data(self, instrument_id, timeframe, limit=100):
        params = {'instId': instrument_id, 'bar': timeframe, 'limit': str(limit)}
        try:
            response = requests.get(self.kline_url, params=params)
            response.raise_for_status()
            data = response.json()
            if data.get('code') == '0':
                return data.get('data', [])
        except requests.exceptions.RequestException as e:
            print(f"\n[ERROR] Failed to get kline for {instrument_id} from OKX: {e}")
        return None

class BinanceAdapter(BaseExchange):
    """Binance 交易所的轉接頭"""
    def __init__(self):
        super().__init__()
        self.name = "Binance"
        self.info_url = "https://api.binance.com/api/v3/exchangeInfo"
        self.kline_url = "https://api.binance.com/api/v3/klines"

    def get_spot_instruments_by_quote(self, quote_currency):
        try:
            response = requests.get(self.info_url)
            response.raise_for_status()
            data = response.json()
            quote = quote_currency.upper()
            return [
                s['symbol'] for s in data['symbols'] 
                if s['status'] == 'TRADING' and s['quoteAsset'] == quote
            ]
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] Binance API request failed: {e}")
        return None

    def get_kline_data(self, instrument_id, timeframe, limit=100):
        timeframe_map = {"15m": "15m", "30m": "30m", "1H": "1h", "4H": "4h", "1D": "1d"}
        api_timeframe = timeframe_map.get(timeframe, "1h")
        params = {'symbol': instrument_id, 'interval': api_timeframe, 'limit': str(limit)}
        try:
            response = requests.get(self.kline_url, params=params)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            print(f"\n[ERROR] Failed to get kline for {instrument_id} from Binance: {e}")
        return None

# --- 分析模組 ---
def analyze_triangle_consolidation(kline_data, period=60):
    if not kline_data or len(kline_data) < period: return False, "K 線數據不足"
    recent_klines = kline_data[:period]
    highs = np.array([float(k[2]) for k in recent_klines])
    lows = np.array([float(k[3]) for k in recent_klines])
    x = np.arange(len(highs))
    highs_slope, _ = np.polyfit(x, highs, 1)
    lows_slope, _ = np.polyfit(x, lows, 1)
    first_half_range = np.max(highs[period//2:]) - np.min(lows[period//2:])
    second_half_range = np.max(highs[:period//2]) - np.min(lows[:period//2])
    is_volatility_decreasing = second_half_range < first_half_range
    if highs_slope < 0 and lows_slope > 0 and is_volatility_decreasing:
        return True, f"符合三角收斂: 高點趨勢向下 (斜率: {highs_slope:.6f}), 低點趨勢向上 (斜率: {lows_slope:.6f})"
    return False, "不符合三角收斂"

def analyze_double_bottom(kline_data, period=60):
    if not kline_data or len(kline_data) < period: return False, "K 線數據不足"
    recent_klines = kline_data[:period]
    lows = np.array([float(k[3]) for k in recent_klines])
    close_prices = np.array([float(k[4]) for k in recent_klines])
    third = period // 3
    low_1_price = np.min(lows[2*third:])
    peak_price = np.max(close_prices[third:2*third])
    low_2_price = np.min(lows[:third])
    are_lows_similar = abs(low_1_price - low_2_price) / low_1_price < 0.03
    avg_low_price = (low_1_price + low_2_price) / 2
    is_peak_significant = (peak_price - avg_low_price) / avg_low_price > 0.05
    is_rebounding = close_prices[0] > low_2_price
    if are_lows_similar and is_peak_significant and is_rebounding:
        return True, f"符合 W 底: 兩個低點約為 {avg_low_price:.4f}, 頸線高點為 {peak_price:.4f}"
    return False, "不符合 W 底"

def analyze_ascending_triangle(kline_data, period=60):
    if not kline_data or len(kline_data) < period: return False, "K 線數據不足"
    recent_klines = kline_data[:period]
    highs = np.array([float(k[2]) for k in recent_klines])
    lows = np.array([float(k[3]) for k in recent_klines])
    x = np.arange(len(highs))
    highs_slope, _ = np.polyfit(x, highs, 1)
    lows_slope, _ = np.polyfit(x, lows, 1)
    is_lows_rising = lows_slope > 0
    is_highs_flat = abs(highs_slope) < (lows_slope * 0.25)
    if is_lows_rising and is_highs_flat:
        return True, f"符合上升三角形: 高點趨于水平 (斜率: {highs_slope:.6f}), 低點趨勢向上 (斜率: {lows_slope:.6f})"
    return False, "不符合上升三角形"

# --- 函式庫/工具箱 ---
ANALYSIS_FUNCTIONS = {
    "triangle": analyze_triangle_consolidation,
    "double_bottom": analyze_double_bottom,
    "ascending_triangle": analyze_ascending_triangle,
}
EXCHANGE_ADAPTERS = {
    "okx": OkxAdapter,
    "binance": BinanceAdapter,
}

# --- 核心掃描函式 ---
def run_scan(exchange, quote, timeframe, pattern, limit=None, progress_callback=None, count_callback=None):
    pairs = exchange.get_spot_instruments_by_quote(quote)
    if not pairs:
        return [f"未能從 {exchange.name} 獲取任何 {quote.upper()} 交易對。"]
    pairs_to_scan = pairs
    if limit:
        pairs_to_scan = pairs[:limit]
    total_pairs = len(pairs_to_scan)
    if count_callback:
        count_callback(total_pairs)
    selected_analysis_func = ANALYSIS_FUNCTIONS[pattern]
    found_list = []
    iterator = tqdm(pairs_to_scan, desc=f"Scanning {exchange.name}") if not progress_callback else pairs_to_scan
    for i, pair in enumerate(iterator):
        kline_data = exchange.get_kline_data(pair, timeframe=timeframe, limit=60)
        if kline_data:
            is_pattern, description = selected_analysis_func(kline_data)
            if is_pattern:
                found_list.append(f"✅ [{exchange.name}] {pair}: {description}")
        if progress_callback:
            progress_callback((i + 1) / total_pairs)
        time.sleep(0.1)
    return found_list

# --- 主程式執行區 ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="市場形態掃描器 (CLI模式)")
    parser.add_argument("quote", help="報價貨幣")
    parser.add_argument("--exchange", default="okx", choices=EXCHANGE_ADAPTERS.keys(), help="要掃描的交易所")
    parser.add_argument("--timeframe", default="1H", help="K 線時框")
    parser.add_argument("--pattern", default="triangle", choices=ANALYSIS_FUNCTIONS.keys(), help="技術形態")
    parser.add_argument("--limit", type=int, help="掃描數量 (預設全部)")
    args = parser.parse_args()
    exchange_class = EXCHANGE_ADAPTERS[args.exchange]
    exchange_adapter = exchange_class()
    results = run_scan(exchange_adapter, args.quote, args.timeframe, args.pattern, args.limit)
    print("\n" + "="*50)
    print("掃描完成！")
    if results:
        print(f"共找到 {len(results)} 個符合條件的目標：")
        for result in results:
            print(result)
    else:
        print("未找到符合條件的目標。")
    print("="*50)